...
def main(cfg: Config) -> None:
    ...
    with mlflow.start_run():
        ...
        mlflow.log_params({"rag_method": cfg.rag.rag_method})
        mlflow.log_input(
        ...
        with mlflow.start_span(name="root"):
            ...
            responses = rag.run(
                ...
                retrieval_queries=[
                    prepare_conversation_history(row, multi_turn_context)
        ...
        json_dump = [
            flatten_dict(response.to_dict(truncated=True)) for response in responses.response_data
        ]
    ...
    for _, row in df.iterrows():
        ...
        for ctx in row["ctxs"]:
            ...
            if text not in seen_contexts:
                contexts.append(Document(content=text, meta_data=MetaData({"title": ctx["title"]})))
